...
export function MeditateDetectVisual({ calm }: { calm: number }) {
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      const c = Math.max(0, Math.min(100, calmRef.current)) / 100;
      const cx = cssW / 2;
      const cy = cssH * 0.48;
      const period = 2800 - c * 700;
      const breath = 0.5 + 0.5 * Math.sin(t / period);
      const rx = 48 + c * 28 + breath * 10;
      const ry = 22 + c * 12 + breath * 6;

      // 底场：软白径向光，calm 越高越沉静明亮
      ...
      for (const m of motes) {
        m.y += m.drift * (0.4 + (1 - c) * 0.8);
        if (m.y > 1.08) m.y = -0.05;
        const px = m.x * cssW + Math.sin(t * 0.0004 + m.phase) * 6;
        const py = m.y * cssH;
        ctx.beginPath();
